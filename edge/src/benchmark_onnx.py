...
def measure_latency(session: ort.InferenceSession, input_data: np.ndarray, num_warmup: int = 10, num_runs: int = 100) -> Dict[str, float]:
...
def measure_throughput(session: ort.InferenceSession, input_data: np.ndarray, duration_seconds: float = 5.0) -> float:
    input_name = session.get_inputs()[0].name
...
def benchmark_model(model_path: str, config_path: str = 'configs/config.yaml') -> Dict:
    ...
    report_path = 'reports/benchmark_fp32.md'
