...
class CNN_BiLSTM(nn.Module):
    def __init__(self, num_leads=12, cnn_channels=[64, 128, 256], cnn_kernel_sizes=[7, 5, 3], lstm_hidden_size=128, lstm_num_layers=2, num_classes=4, dropout=0.3):
    ...
    def forward(self, x):
        # x: (batch, 12, 5000)
        x = self.cnn(x)
        # 2. Permute: (batch, channels, seq) -> (batch, seq, channels)
        x = x.permute(0, 2, 1)
        # BiLSTM
        lstm_out, (hn, cn) = self.lstm(x)
        # 4. Take the last hidden state output
        x = lstm_out[:, -1, :]
        # Classifier
        x = self.classifier(x)
        return x
